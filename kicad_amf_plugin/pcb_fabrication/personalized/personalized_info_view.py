...
class PersonalizedInfoView(UiPersonalizedService, FormPanelBase):
    ...
    def get_from(self, kind: FormKind) -> 'dict':
        info = PersonalizedInfoModel(
            ...
            review_file=REVIEW_FILE_OPTION[int(
                self.comb_approve_gerber.GetSelection())].EditRole,
            ...
            cross_board=CROSS_BOARD[self.combo_cross_board.GetSelection(
            )].EditRole,
            paper=PAPER[self.combo_paper.GetSelection()].EditRole,
            user_stamp=USER_STAMP[self.combo_user_stamp.GetSelection()
                                  ].EditRole,

            hq_pack=HQ_PACK[int(self.combo_hq_pack.GetSelection(
            ))].EditRole if self.combo_hq_pack.Shown else None,
            pcb_note=self.edit_special_request.GetValue(
            ) if self.edit_special_request.Shown else None,
